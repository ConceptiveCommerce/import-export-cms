...
class Content implements ContentInterface
{
    ...
    public function importFromZipFile($fileName, $rm = false): int
    {
        ...
        if ($res !== true) {
            $message = __('Cannot open ZIP archive');
                $this->messageManager->addWarning($message); return 0;
        }

        $subPath = hash('sha256', date(DATE_RFC2822));
        ...
        if (!$this->file->fileExists($pagesFile, true)) {
            //throw new \Exception("Please Add " . self::JSON_FILENAME . ' in ZIP Archive');
            $message = __('Please Add cms.json in ZIP Archive');
                $this->messageManager->addWarning($message);return 0;
        }
    ...
    public function importFromArray(array $payload, $archivePath = null): int
    {
        if (!isset($payload['pages']) && !isset($payload['blocks'])) {
           // throw new \Exception('Invalid json archive');
            $message = __('Invalid json archive');
                $this->messageManager->addWarning($message);return 0;
        }
